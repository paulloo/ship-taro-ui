...
import { TaroDateProps, TaroDateState } from 'types/taro-date'
...
export default class TaroDates extends AtComponent<TaroDateProps, TaroDateState> {
  ...
  isLastDayOfWeek(day) {
    const { firstDayOfWeek } = this.props;
    return day.day() === ((firstDayOfWeek || dayjs().startOf('week').day()) + 6) % 7;
  }

  isSaturdayOfWeek(day) {
    const { firstDayOfWeek } = this.props;
    return day.day() === ((firstDayOfWeek || dayjs().startOf('week').day()) + 5) % 7;
  }
  ...
  doesNotMeetMinimumNights(day) {
    ...
    if (startDate) {
      const dayDiff = day && day.diff(startDate.clone().startOf('day').hour(12), 'day');
  ...
  onDayClick(day, e) {
    ...
    if (startDateOffset || endDateOffset) {
      ...
      const lastAllowedStartDate = endDate && endDate.clone().subtract(minimumNights, 'day');
        ...
        <View className='cal-week-header' style={{top: fromTop}}>
          {
            WEEKDAYS.map(i => {
              return (
                <View key={i} className={classnames('cal-week-item', {
                  'cal-week-item__active': (i === 0 || i === 6)
                })}
                >
                  <Text>{dayjs().day((i + firstDayOfWeek) % 7).format(weekDayFormat)}</Text>
                </View>
              )
            })
          }
